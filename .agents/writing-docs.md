...
Act whenever a promoted skill is added, renamed, or has its behaviour changed: create or re-sync its docs page. A rename moves the file too (`docs/<bucket>/<old>.md` to `docs/<bucket>/<new>.md`); a skill that moves between `engineering/` and `productivity/` moves its docs file to the matching folder, and fix the relative links in it. Skills in `misc/`, `in-progress/`, and `deprecated/` get no page, because none of those buckets is promoted. A skill moving *out* of one of them into `engineering/` or `productivity/` gains a page; one moving the other way loses it.

Because these pages live in the repo, **every link is relative**: a sibling page in the same bucket is `./<name>.md`, in the other bucket `../<bucket>/<name>.md`, and a skill supporting file is `../../skills/<bucket>/<name>/<file>`. Never an absolute URL for something this repo contains.

There is no H1. The skill name in the link is the title.
...
**A page carries no install commands.** Install wording lives in [the install block](./install-block.md) and is copied into `README.md` only. A page that writes commands out duplicates the README, and the two copies drift.
...
Optional: include only when the skill needs something in place to be functional; omit the heading entirely otherwise. Covers: a **workspace it writes into** (a stateful skill like `grill-with-docs` writes `CONTEXT.md` and ADRs; `teach` builds a whole directory, so say what it writes and where), **prior setup** (`triage`/`to-spec`/`to-tickets` need `setup` to have configured an issue tracker), or **repo-specific tooling**. A stateless skill that runs anywhere has no prerequisites, so drop the section.
...
The single non-negotiable: **surface the skill's leading word / defining idea** (`tight` feedback loop, `deep module`, throwaway-code-answers-a-question, red-green). It pays off twice: the reader learns what the skill *is*, and learns the word they will later think with to *reach for* it.
...
- **This repo's issues.** `gh issue list --repo wxitzxg/fore --search "<skill-name>" --state all`. A question filed twice is a question the page owes an answer to.
- **Upstream issues.** fore is forked from `mattpocock/skills`; its issue history covers the years before the fork and is worth searching the same way. Attribute findings to "one user reported", never to a named author.
...
- **Role.** Name it: a **chain step** (`grill-with-docs -> to-spec -> to-tickets -> implement -> code-review`), a **run-once setup** (`setup`), **periodic maintenance** (`improve-codebase-architecture`, "every few days"), or a **reach-for-it-anytime standalone** (`diagnosing-bugs`, `prototype`, `handoff`). A standalone's map is one honest sentence, which is far better than omitting the section.
- **Neighbours.** The one or two siblings that matter, each with a because-clause, linked relatively.
- **The map.** Point to [guide](../docs/engineering/guide.md), the router over the whole set, so this page stays a node and never has to redraw the graph.
...
- The hunt for real questions ran (the issues, the changelog), and `## Common questions` is sized to what it found, not padded to match a richer skill's page.
- Every `## It's working if` bullet is checkable without opening `SKILL.md`.
- The sections appear in the template's order.
- Every link is relative, and every one resolves.
