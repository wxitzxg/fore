...
The map and its tickets live on the repo's issue tracker, so wayfinder needs the tracker wiring that [setup](./setup.md) lays down. That step writes a "Wayfinding operations" section describing how the map, its child tickets, blocking edges, and frontier queries are expressed for GitHub, GitLab, or local markdown. Wayfinder resolves that doc through the pointer in your `CLAUDE.md` / `AGENTS.md` rather than a fixed path; with no tracker configured at all it falls back to local markdown files.
...
The most-reported failure with this skill, and there was a real hole behind it: wayfinder's "plan, don't do" default could be overridden in the map's **Notes**, but the Notes are written by the agent, so the constraint and its exemption lived in the same file the constrained party owned. One user watched an agent write "this map carries execution" into its own Notes and read it back in later sessions as its own licence, building on a live server. The skill now carries explicit guardrails against this family: the execution override counts only when the user has written it into the Notes themselves, never when the agent infers it; an admission test rejects destination work at ticket creation; and the session that empties the decision frontier stops and hands you to `/to-spec` instead of collapsing the spec itself. Residual advice on a map you didn't chart: read its Notes before working it, keep implementation in its own sessions, and treat any `wayfinder:task` that looks like a slice of the build as mis-typed.
